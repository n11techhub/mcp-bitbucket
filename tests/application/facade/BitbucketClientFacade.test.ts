...
describe('BitbucketClientFacade', () => {
    ...
    it('delegates remaining methods to underlying clients', async () => {
        ...
        pullRequestClient.declineBitbucketPullRequest.mockResolvedValue({ ok: 3 });
        pullRequestClient.addBitbucketGeneralPullRequestComment.mockResolvedValue({ ok: 4 });
        pullRequestClient.getBitbucketPullRequestDiff.mockResolvedValue({ ok: 5 });
        pullRequestClient.getBitbucketPullRequestReviews.mockResolvedValue({ ok: 6 });
        pullRequestClient.addBitbucketPullRequestFileLineComment.mockResolvedValue({ ok: 7 });
        repositoryClient.listBitbucketRepositories.mockResolvedValue({ ok: 8 });
        repositoryClient.listBitbucketRepositoryBranches.mockResolvedValue({ ok: 9 });
        repositoryClient.createBitbucketBranch.mockResolvedValue({ ok: 10 });
        repositoryClient.getBitbucketFileContent.mockResolvedValue({ ok: 11 });
        repositoryClient.browseBitbucketDirectory.mockResolvedValue({ ok: 12 });
        repositoryClient.getBitbucketRepositoryDetails.mockResolvedValue({ ok: 13 });
        searchClient.searchBitbucketContent.mockResolvedValue({ ok: 14 });
        userClient.getBitbucketUserDetails.mockResolvedValue({ ok: 15 });
        ...
        await expect(facade.declineBitbucketPullRequest(prParams as any, 'decline')).resolves.toEqual({ ok: 3 });
        await expect(facade.addBitbucketGeneralPullRequestComment(prParams as any, { text: 'hi' } as any)).resolves.toEqual({ ok: 4 });
        await expect(facade.getBitbucketPullRequestDiff(prParams as any, 20)).resolves.toEqual({ ok: 5 });
        await expect(facade.getBitbucketPullRequestReviews(prParams as any)).resolves.toEqual({ ok: 6 });
        await expect(facade.addBitbucketPullRequestFileLineComment({ workspaceSlug: 'N11', repoSlug: 'repo', prId: 1, content: 'ok' } as any)).resolves.toEqual({ ok: 7 });
        await expect(facade.listBitbucketRepositories({ query: 'repo' } as any)).resolves.toEqual({ ok: 8 });
        await expect(facade.listBitbucketRepositoryBranches({ workspaceSlug: 'N11', repoSlug: 'repo' } as any)).resolves.toEqual({ ok: 9 });
        await expect(facade.createBitbucketBranch({ workspaceSlug: 'N11', repoSlug: 'repo', newBranchName: 'feat/x' } as any)).resolves.toEqual({ ok: 10 });
        await expect(facade.getBitbucketFileContent({ workspaceSlug: 'N11', repoSlug: 'repo', filePath: 'a.ts' } as any)).resolves.toEqual({ ok: 11 });
        await expect(facade.browseBitbucketDirectory({ workspaceSlug: 'N11', repoSlug: 'repo' } as any)).resolves.toEqual({ ok: 12 });
        await expect(facade.getBitbucketRepositoryDetails({ workspaceSlug: 'N11', repoSlug: 'repo' } as any)).resolves.toEqual({ ok: 13 });
        await expect(facade.searchBitbucketContent({ workspaceSlug: 'N11', query: 'x' } as any)).resolves.toEqual({ ok: 14 });
        await expect(facade.getBitbucketUserDetails({ username: 'john' } as any)).resolves.toEqual({ ok: 15 });
    });
